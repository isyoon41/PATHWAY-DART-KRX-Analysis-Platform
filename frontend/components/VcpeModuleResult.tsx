...
function SignalCard({ signal, positive }: { signal: VcpeSignal; positive: boolean }) {
  const dot = IMPORTANCE_DOT[signal.importance || 'medium'] ?? IMPORTANCE_DOT.medium;
  return (
    <div className={`p-3 rounded border ${positive ? 'border-emerald-200 bg-emerald-50' : 'border-red-200 bg-red-50'}`}>
      <div className="flex items-start gap-2">
        ...
        <div>
          <p className="text-[13px] font-semibold text-[#1E293B]">{signal.signal}</p>
          {signal.evidence && (
            <p className="text-[11px] text-[#64748B] mt-0.5">{signal.evidence}</p>
          )}
        </div>
      </div>
    </div>
...
export default function VcpeModuleResult({ data, moduleName }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── 시그널 (Positive / Negative) ── */}
      {((data.positive_signals?.length ?? 0) > 0 || (data.negative_signals?.length ?? 0) > 0) && (
        <div className="grid grid-cols-2 gap-4">
          {/* Positive */}
          <div>
            <h3 className="text-[12px] font-bold uppercase tracking-widest text-[#64748B] flex items-center gap-1.5 mb-2">
